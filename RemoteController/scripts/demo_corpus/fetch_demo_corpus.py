...
Examples
--------
    python scripts/demo-corpus/fetch_demo_corpus.py build --out ~/demo-corpus
    python scripts/demo-corpus/fetch_demo_corpus.py build --out ~/demo-corpus --only caselaw,slds
    python scripts/demo-corpus/fetch_demo_corpus.py verify --out ~/demo-corpus
    python scripts/demo-corpus/fetch_demo_corpus.py upload --out ~/demo-corpus \\
        --host rc-demo.example.ch --remote-path /srv/knovas/documents --execute
"""
...
try:
    ...
    sys.exit("missing dependencies — run: pip install -r scripts/demo-corpus/requirements.txt")
...
SELECT DISTINCT ?srNotation ?title ?dateApplicability ?pdfUrl WHERE {
  ...
  ?caExpr jolux:language <http://publications.europa.eu/resource/authority/language/%(lang_uri)s> ;
          jolux:title ?title .
  ?cons jolux:isMemberOf ?ca ;
        jolux:dateApplicability ?dateApplicability ;
        jolux:isRealizedBy ?expr .
  ?expr jolux:language <http://publications.europa.eu/resource/authority/language/%(lang_uri)s> ;
...
def main() -> int:
    ...
    upload = sub.add_parser("upload", help="Korpus auf den Remote-Controller-Host schieben")
