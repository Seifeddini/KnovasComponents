...
| `no extractable text from .pdf file` | Image-only scan. Rebuild RC with Tesseract + `knovas-extract>=0.3` (Dockerfile installs from git until PyPI has 0.3.0). Re-queue skips: `DELETE FROM documents WHERE transmission_key_id LIKE 'skip:%';` in `.rc-sync-state.db` |
