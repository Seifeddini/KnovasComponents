...
class TestRcContract:
    ...
    def test_discover_anonymous_never_200(self, rc_client):
        with patch.dict(os.environ, {"RC_MTLS_DEV_BYPASS": "false"}):
    ...
    def test_discover_returns_200_when_authorized(self, rc_client):
    ...
    def test_discover_rejects_unauthorized_verify(self, rc_client):
        with patch("auth.knovas_verify_client.get_verify_client") as mock_client:
            mock_client.return_value.verify_operator.return_value = (
                False,
                None,
    ...
    def test_discover_ip_rate_limit_burst_429(self, rc_client):
        with patch.dict(os.environ, {"RC_MTLS_DEV_BYPASS": "false"}):
            with patch("auth.rc_rate_limit._get_ip_limiter") as mock_lim:
                ...
                last_status = None
                for _ in range(6):
                    last_status = rc_client.get("/discover").status_code
                assert last_status == 429

    def test_sync_requires_json(self, rc_client):
        with patch("auth.knovas_verify_client.get_verify_client") as mock_client:
    ...
    def test_discover_verify_unreachable_503(self, rc_client):
        with patch("auth.knovas_verify_client.get_verify_client") as mock_client:
            mock_client.return_value.verify_operator.return_value = (
                False,
                None,
    ...
    def test_discover_passes_mtls_identity_to_verify(self, rc_client):
        with patch("auth.knovas_verify_client.get_verify_client") as mock_client:
            mock_client.return_value.verify_operator.return_value = (
                True,
                "22222222-2222-2222-2222-222222222222",
            ...
            _jwt, employee_id, serial = mock_client.return_value.verify_operator.call_args[0]
            assert employee_id == "11111111-1111-1111-1111-111111111111"
            assert serial == "rc-dev-bypass"
