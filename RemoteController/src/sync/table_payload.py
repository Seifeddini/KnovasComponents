...
def map_extractor_tables(
    ...
    *,
    ...
    """Convert extractor `content.tables` entries to API-ready dicts (unassigned to parts)."""
    if not raw_tables:
        return []
    ...
    for idx, raw in enumerate(raw_tables):
        ...
        if isinstance(bbox, (list, tuple)) and len(bbox) == 4:
            try:
                ...
                pass
        offset = _table_attr(raw, "char_start", "offset", "start")
        if offset is not None:
            try:
                item["_char_start"] = int(offset)
            except (TypeError, ValueError):
                pass
        mapped.append(item)
    return mapped
...
def assign_tables_to_parts(
    ...
    *,
    ...
    """Attach up to 50 tables per part onto in-place part dicts (`tables` key)."""
    ...
    for table in tables:
        api_table = {k: v for k, v in table.items() if not str(k).startswith("_")}
        offset = table.get("_char_start")
        part_idx = 0
        if offset is not None:
            for i, (b_start, b_end) in enumerate(boundaries[: len(parts)]):
                if b_start <= int(offset) < b_end:
                    part_idx = i
                    break
        elif table.get("page") is not None:
            page = table["page"]
        ...
        if len(buckets[part_idx]) < _MAX_TABLES_PER_PART:
            buckets[part_idx].append(api_table)
