...
echo "volume, so it survives a container recreate). Read it with:"
echo "  docker compose --env-file knovas.env exec docbridge-web cat /app/data/platform-admin-bootstrap"
echo ""
echo "Lost it, or want to set one yourself? ./scripts/admin-password.sh"
