...
BROKER_KEY_DIR="$(read_env_var PLATFORM_BROKER_KEY_DIR "/app/data/broker_keys" "$KNOVAS_ENV")"
...
echo "  Broker keys:  $BROKER_KEY_DIR (created in the container on the data volume)"
echo ""
echo "On first start the Platform applies identity migrations, creates the"
echo "administrator account, and writes a one-time password to"
