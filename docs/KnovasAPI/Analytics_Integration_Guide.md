...
Who this is for: software engineers integrating **Knovas** through the secured HTTPS APIs (`/secured/query`, `/secured/analytics/engagement`, `/secured/analytics/relevance-feedback`, `/secured/document/rating`).

When to use: after your search integration is working, add engagement reporting and/or explicit feedback to help Knovas measure and improve search quality.

Knovas supports two kinds of client-reported feedback:
...
Every `/secured/query` success payload includes **`query_session_id`** at the **top level** (flat envelope; no nested `data` wrapper). Example and field list: [Secure API — query](Secure_API.md#post-securedquery).

Store this `query_session_id` alongside the search results you display to your users. **`results[]`** entries also expose **`document_uuid`** and **`ingested_summary`** — see [Secure API — query response fields](Secure_API.md#post-securedquery).
...
Invalid events are dropped; `accepted` counts how many were stored.

**Error responses:**

| Status | Condition |
|--------|-----------|
| `400` | Missing `query_session_id`, empty events array, or events exceeds 50 |
...
## What Knovas tracks automatically
...
- If your search UI re-ranks or filters results before displaying them to the user, send the **displayed position**, not the Knovas-returned position.
- Position is optional but highly valuable for search quality measurement.

## Privacy guarantees

- Knovas does not store your query text in analytics.
...
Strongly recommended but not required. Knovas works fully without it. However, engagement data enables search quality measurement and future relevance tuning.
